...
function keySequence(node: ManualNode): string | undefined {
  ...
  const icon = (name?: string): string => name === "shift"
    ...
    : name === "backspace"
      ...
      : "";
  return `<span class="manual-key-sequence">${presentation.keys.map((key) => `<kbd class="manual-key manual-key-${escape(key.variant ?? "regular")}">${icon(key.icon)}<span>${escape(key.label ?? "")}</span></kbd>`).join('<span class="manual-key-plus" aria-hidden="true">+</span>')}</span>`;
}
...
function renderNode(node: ManualNode, context: RenderContext): string {
  switch (node.type) {
    ...
    case "inlineCode": return `<code${isCommandLine(node) ? ' class="manual-command-line"' : ""}>${escape(node.value ?? "")}</code>`;
