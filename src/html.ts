...
function renderKeys(keys: ManualKey[]): string {
  ...
  return keys.map((key) => `<kbd class="manual-key manual-key-${escape(key.variant ?? "regular")}" aria-label="${escape(accessibleLabel(key))}">${icon(key.icon)}${key.label ? `<span>${escape(key.label)}</span>` : ""}</kbd>`).join('<span class="manual-key-plus" aria-hidden="true">+</span>');
}
