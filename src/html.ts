...
function renderKeys(keys: ManualKey[]): string {
  const icon = (name?: string): string => name === "shift"
    ...
    : name === "backspace"
      ...
      : "";
  return keys.map((key) => `<kbd class="manual-key manual-key-${escape(key.variant ?? "regular")}">${icon(key.icon)}<span>${escape(key.label ?? "")}</span></kbd>`).join('<span class="manual-key-plus" aria-hidden="true">+</span>');
}
...
function controlSequence(node: ManualNode): string | undefined {
  ...
  return `<code class="manual-control-sequence">${presentation.segments.map((segment) => segment.keys ? renderKeys(segment.keys) : escape(segment.text ?? "")).join("")}</code>`;
}
...
function css(config: ResolvedConfig): string {
  ...
  const callouts = `.manual-page h3:after{height:.32rem}.callout{background:var(--callout-background,#eef8f8)}.manual-key{min-height:1.8rem;padding:.28rem .6rem;border-color:#59636d;border-radius:.58rem;box-shadow:0 2px 2px #0006,inset 0 1px #ffffff24}.manual-command-line{padding:.25rem .55rem;border-radius:.55rem}.manual-control-sequence{display:inline-flex;align-items:center;gap:.25rem;margin:0 .08rem;padding:.3rem .42rem;border:1px solid #334554;border-radius:.62rem;background:var(--code);color:#f4f7f9;box-shadow:inset 0 1px #ffffff18,0 1px 2px #0004;font:700 .86em/1.35 ui-monospace,SFMono-Regular,Menlo,monospace;white-space:nowrap}.manual-control-sequence .manual-key{font-size:.9em}${Object.entries(t.callouts).map(([kind, color]) => `.callout-type-${kind}{--callout-color:${color}}`).join("")}`;
